...
// -- Handle "Add new subscriber" form
$newError = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_email'], $_POST['new_brand'])) {
    ...
    if ($email && $brand) {
        // Check if exists
        $existsStmt = $pdo->prepare("SELECT id FROM newsletter_subscriptions WHERE email = :email AND brand_slug = :brand LIMIT 1");
    ...
    // reload to avoid form resubmission
...
// -- Sort
$validSortColumns = ['email', 'brand_slug', 'status', 'created_at', 'confirmed_at'];
$sort = filter_input(INPUT_GET, 'sort', FILTER_SANITIZE_STRING);
$sort = in_array($sort, $validSortColumns, true) ? $sort : 'created_at';
...
// Count total
$countSql = "SELECT COUNT(*) as cnt FROM newsletter_subscriptions
...
<form method="get" class="mb-3">
  <div class="row">
    <div class="col-md-4">
      <input type="text" name="search" class="form-control" placeholder="Suchen (Email oder Brand)" value="<?= htmlspecialchars($search) ?>">
    </div>
    <div class="col-md-2">
